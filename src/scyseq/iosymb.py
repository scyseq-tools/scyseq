...
def read_codix(fname, data_only=True):
    ...
    """
    with open(fname, 'r') as datafile: record = json.load(datafile)
...
def write_codix(fname, seqs, info=None):
    ...
    container = {'history': [], # [(date, observer, comment), ...]
    ...
    with open(fname, 'w') as datafile: json.dump(container, datafile)
